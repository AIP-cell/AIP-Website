...
export const addProgramsGallery = async (req, res, next) => {
	try {
			...
			if (file.fieldname.startsWith('galleryImage')) {
				...
				for (let index = 0; index < galleryLength; index++) {
					if (file.fieldname.startsWith(`galleryVideo[${index}]`)) {
						...
						// Check if linkOrVideo is a video or a link
						if (videoEntry.linkOrVideo === 'video') {
							...
							videoEntry.link = videoEntry.linkOrVideo; // Assign link if it's not a video
						}
					}
				}
			}
		...
		await ProjectAndProgramModel.updateOne(
			{ _id: programId },
			{
				$set: { gallery: gallery, updatedBy: userId },
			},
		);
...
export const geProjectGalleryById = async (req, res) => {
	try {
		const Id = new mongoose.Types.ObjectId(req.params.id);
		const gallery = await ProjectAndProgramModel.aggregate([
			{
				$match: {
		...
		return res.status(200).json({
			...
			data: { gallery: gallery.at(0).gallery },
		});
	} catch (error) {
