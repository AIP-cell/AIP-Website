...
export const addNpoPeople = async (req, res, next) => {
	try {
...
export const updateNpoPeople = async (req, res, next) => {
	try {
		const Id = req.params.id;
		const { name, designation, tags } = req.body;

		const { userId } = req.user;
		const people = await NpoModel.findOne({ 'people._id': Id, deletedAt: null });
...
export const deletePeople = async (req, res) => {
	try {
		...
		const result = await NpoModel.updateOne({ 'people._id': Id }, { $pull: { people: { _id: Id } } });
