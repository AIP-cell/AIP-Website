...
export const getCollaborationTestimonialData = async (req, res, next) => {
	try {
		...
		const aggregationPipeline = [
			...
			},
			{
				$match: searchTerm
					? {
							$or: [{ 'testimonials.name': { $regex: searchTerm, $options: 'i' } }],
						}
					: {},
			},
			{
		...
		const testimonials = await CollaborationCityModel.aggregate(aggregationPipeline);
