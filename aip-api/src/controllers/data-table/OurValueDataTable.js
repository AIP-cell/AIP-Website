import { AuthModel } from '../../modals/AuthModel.js';
import { OurValuesModel } from '../../modals/OurValuesModal.js';
import { serverError } from '../../utils/errorHandler.js';

export const getOurValueData = async (req, res, next) => {
	try {
		const page = Math.abs(Number.parseInt(req.query.page) || 1);
		const pageSize = 15;
		const searchTerm = req.query.q;

		const aggregationPipeline = [
			{
				$match: { deletedAt: null },
			},
			{
				$sort: { createdAt: -1 },
			},
			{
				$lookup: {
					from: AuthModel.modelName,
					localField: 'updatedBy',
					foreignField: '_id',
					as: 'lastUpdated',
				},
			},
			{
				$unwind: {
					path: '$lastUpdated',
				},
			},
			{
				$project: {
					_id: 1,
					description: 1,
					title1: 1,
					title2: 1,
					isActive: 1,
					updatedBy: '$lastUpdated.name',
				},
			},
			{
				$facet: {
					paginatedResults: [{ $skip: (page - 1) * pageSize }, { $limit: pageSize }],
					totalCount: [
						{
							$count: 'count',
						},
					],
				},
			},
		];
		if (searchTerm) {
			aggregationPipeline.unshift({
				$match: {
					$and: [
						{ deletedAt: null },
						{
							$or: [{ title1: { $regex: searchTerm, $options: 'i' } }],
						},
					],
				},
			});
		}

		const ourValues = await OurValuesModel.aggregate(aggregationPipeline);

		if (!ourValues || ourValues.length === 0) {
			return res.status(200).json({
				success: false,
				message: 'No our values found',
				data: {},
			});
		}
		const [result] = ourValues;
		return res.status(200).json({
			success: true,
			message: 'Our values  data fetched',
			data: {
				ourValues: result.paginatedResults,
				totalCount: result.totalCount.at(0)?.count,
				currentPage: page,
			},
		});
	} catch (error) {
		return next(serverError(error));
	}
};
