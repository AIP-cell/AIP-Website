"use client";

import EditNpoPeoplePage from "@/app/(admin)/_shared/components/npo-people/EditPage";
import NpoCaseStudiesApi from "@/axios/NpoCaseStudiesApi";
import NpoPeopleApi from "@/axios/NpoPeopleApi";
import { reqAuthHeader } from "@/utils/reqAuthHeader";
import { useQuery } from "@tanstack/react-query";
import { ClipLoader } from "react-spinners";
export const dynamic = "force-dynamic";
const defaultValues = async (id: string) => {
  try {
    console.log("first");
    const response = await NpoPeopleApi.view(id, reqAuthHeader());

    return response.data.data.people;
  } catch (error) {
    console.log(error);
  }
  return null;
};

interface Params {
  params: {
    id: string;
  };
}

const Edit: React.FC<Params> = ({ params }) => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["npo", params.id],
    queryFn: () => defaultValues(params.id),
    gcTime: 0,
  });
  if (isLoading) {
    return (
      <div
        style={{
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          height: "100vh",
        }}
      >
        <ClipLoader size={50} color={"#5E165B"} loading={isLoading} />
      </div>
    );
  }

  if (isError || !data) {
    return null;
  }

  return <EditNpoPeoplePage values={data} />;
};

export default Edit;
