...
const ContactFormTable = () => {
      ...
      try {
        await dataTableApi
          ...
          .then((res) => {
            ...
            setRows(res.data.data?.contacts);
          });
      } catch (err) {
  ...
  const deleteContactForm = (id: any) => {
    setOpenDeleteDialog(true);
    setDialogId(id);
  };

  const deleteContactFormData = async (id: any) => {
    if (buttonRef.current) {
      buttonRef.current.disabled = true;
    }

    try {
      const response = await ContactFormApi.delete(id, reqAuthHeader());

      if (response.data.success) {
        toast.success(response.data.message);
        router.push("/admin/contact-form");
        fetchTableData("asc", query);
      } else {
      ...
      renderCell: ({ row }: { row: any }) => (
        ...
        >
          ...
          <Tooltip title="Delete">
            <IconButton onClick={() => deleteContactForm(row._id)} size="small">
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/contact-form/view/` + row._id}
            >
    ...
    <>
      ...
      {openDeleteDialog && (
        <DeleteConfirmationDialog
          ...
          deleteFunction={deleteContactFormData}
        />
      )}
    </>
  );
