...
export const AddAipFounderMethodsOfJoiningPage = () => {
  const {
    register,
    reset,
    control,
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await MethodsApi.add(values, reqAuthHeader());
...
export default AddAipFounderMethodsOfJoiningPage;
