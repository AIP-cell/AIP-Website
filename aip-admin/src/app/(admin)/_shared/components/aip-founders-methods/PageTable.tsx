...
const AipFounderNetworkMethodTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await MethodsApi.delete(id, reqAuthHeader());
  ...
  //   try {
  //     const response = await MethodsApi.(id)
  //     toast.success(response?.data?.message)
  //     fetchTableData("asc", query);
  //   } catch (err) {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              ...
              component={Link}
              href={
                `/admin/aip-founders-network/methods-of-joining/edit/` + row._id
              }
            >
  ...
  return (
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 4 }}>
        <Link href={`/admin/about-us/aip-founder-network`}>
          <Typography>AIP Founder Network</Typography>
        </Link>
        <Typography>Methods of Joining</Typography>
      </Breadcrumbs>
      <Card>
        <CardHeader
          title="AIP Founder Network Method of Joining"
          action={
            <div>
              <Button
                size="medium"
                variant="contained"
                component={Link}
                href={`/admin/aip-founders-network/methods-of-joining/add`}
              >
                Add Methods of Joining
              </Button>
              &nbsp;
            </div>
          }
...
export default AipFounderNetworkMethodTable;
