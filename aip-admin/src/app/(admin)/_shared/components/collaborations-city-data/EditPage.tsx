...
export const EditCollaborationsCityDataByPage = (
  { values, data }: any,
  {
    popperPlacement,
  }: {
  ...
  const {
    ...
    defaultValues: {
      name: values.name,
      organisationDetails: values.organisationDetails,
      catchupDetails: values.catchupDetails,
      link: values.link,
      fromDate: dayjs(values.fromDate).toDate(),
      toDate: dayjs(values.toDate).toDate(),
      ...
      linkOrVideo: values.linkOrVideo,
    },
  });

  const collabId = values.collaborationId;
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await CollaborationCityApi.updateCasestudies(id, values);

      if (response.data.success) {
        toast.success(response.data.message);
        router.push(`/admin/collaborations/city/` + collabId);
      } else {
  ...
  return (
    <>
      <Breadcrumbs sx={{ mb: 4 }}>
        <Link href={`/admin/collaborations/city/` + collabId}>
...
export default EditCollaborationsCityDataByPage;
