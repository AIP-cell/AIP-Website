...
const CityDataTable = () => {
  ...
  const fetchTableData = useCallback(
    async (sort: SortType, q: string) => {
      try {
        await dataTableApi
          .getCollaborationCityTable({
            query: { sort, q, page: paginationModel.page + 1, id: id },
          })
          .then((res) => {
            setTotal(res.data.data?.totalCount);
            setRows(res.data.data?.cities);
          });
      } catch (err) {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await CollaborationCityApi.delete(id, reqAuthHeader());
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/agenda/${row._id}/${id}`}
              >
                Manage
              </Button>
            </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/media/${row._id}/${id}`}
              >
                Manage
              </Button>
            </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/gallery/` + row._id}
              >
                Manage
              </Button>
            </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/testimonial/${row._id}/${id}`}
              >
                Manage
              </Button>
            </Box>
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/collaborations/city/edit/` + row._id}
            >
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/city/add/` + id}
              >
                Add City
              </Button>
              &nbsp;
