...
const ProgramsTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await ProjectAndProgramsApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await ProjectAndProgramsApi.updateStatusCasestudies(
        id,
        reqAuthHeader()
      );
      toast.success(response?.data?.message);
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/gallery/` + row._id}
              >
                Manage
              </Button>
            </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/media/` + row._id}
              >
                Manage
              </Button>
            </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/testimonial/` + row._id}
              >
                Manage
              </Button>
            </Box>
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/projects-programs/edit/` + row._id}
            >
    ...
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 4 }}>
        <Link href={`/admin/projects-programs`}>
          <Typography>Projects Programs</Typography>
        </Link>
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/add`}
              >
                Add Programs
              </Button>
              &nbsp;
