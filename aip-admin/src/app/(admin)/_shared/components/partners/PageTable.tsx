...
const PartnerTable = () => {
      ...
      try {
        await dataTableApi
          ...
          .then((res) => {
            ...
            setRows(res.data.data?.partners);
          });
      } catch (err) {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await PartnerApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await PartnerApi.updateStatus(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/partners/edit/` + row._id}
            >
