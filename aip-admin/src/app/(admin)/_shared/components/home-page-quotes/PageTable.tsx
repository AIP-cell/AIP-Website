...
const HomePageQuoteTable = () => {
  ...
  const deleteQuoteData = async (id: any) => {
    console.log(id);
    ...
    try {
      const response = await QuoteApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await QuoteApi.status(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
      ...
      renderCell: ({ row }: { row: any }) => (
        ...
        >
          ...
          <Tooltip title="Delete">
            <IconButton onClick={() => deleteQuote(row._id)} size="small">
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/home/quotes/edit/` + row._id}
            >
    ...
    <>
      ...
      {openDeleteDialog && (
        <DeleteConfirmationDialog
          ...
          deleteFunction={deleteQuoteData}
        />
      )}
    </>
  );
...
export default HomePageQuoteTable;
