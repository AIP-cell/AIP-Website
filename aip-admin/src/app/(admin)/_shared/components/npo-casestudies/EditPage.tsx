...
export const EditNpoCasestudiesPage = (values: any) => {
  console.log(values.values.galleryVideos);
  const {
    register,
    control,
    reset,
    ...
    defaultValues: {
      ...
      fileLink: values.values.fileLink,
      videoLink: values.values.videoLink,
      video: values.values.video,
      file: values.values.file,
      linkOrVideo: values.values.linkOrVideo,
      isLinkOrPdf: values.values.isLinkOrPdf,
      image: values.values.image,
      galleryImage: values.values.galleryImages?.map((image: any) => ({
        date: dayjs(image.date).toDate(),
        title: image.title,
        image: image.image,
      })),
      galleryVideo: values.values.galleryVideos?.map((video: any) => ({
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await NpoCaseStudiesApi.updateCasestudies(
        id,
        values,
        reqAuthHeader()
      );
            ...
            <Grid container spacing={6}>
              ...
              <Grid item xs={12} md={12}>
                <CustomizedAipDropDown
                  ...
                  register={register}
                  errors={errors}
                  value={values.values.linkOrVideo}
                  watch={watch}
...
export default EditNpoCasestudiesPage;
