...
const AipResourcesTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await AipResourceApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await AipResourceApi.updateStatus(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
  ...
  const handleFeaturedChange = async (id: any) => {
    try {
      const response = await AipResourceApi.updateIsFeteatured(
        id,
        reqAuthHeader()
      );
      toast.success(response?.data?.message);
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/aip-resources/edit/` + row._id}
            >
...
export default AipResourcesTable;
