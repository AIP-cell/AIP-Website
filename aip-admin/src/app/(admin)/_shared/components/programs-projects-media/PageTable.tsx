...
const ProgramMediaTable = () => {
  ...
  const fetchTableData = useCallback(
    async (sort: SortType, q: string) => {
      try {
        await dataTableApi
          .getProjectAndProgramMediaTable({
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await ProjectAndProgramsMediaApi.delete(
        id,
        reqAuthHeader()
      );
      ...
      if (response.data.success) {
        ...
        router.push(`/admin/projects-programs/media/` + id);
        fetchTableData("asc", query);
      } else {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/projects-programs/media/edit/` + row._id}
            >
    ...
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 4 }}>
        <Link href={`/admin/projects-programs`}>
          <Typography>Project & Programs</Typography>
        </Link>
        <Typography>Manage</Typography>
      </Breadcrumbs>
      <Card>
        <CardHeader
          title="Project & Programs Media"
          action={
            <div>
              <Button
                size="medium"
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/media/add/` + id}
              >
                Add Media
              </Button>
              &nbsp;
...
export default ProgramMediaTable;
