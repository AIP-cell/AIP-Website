...
export const AddProgramMediaPage = ({
  popperPlacement,
}: {
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await ProjectAndProgramsMediaApi.add(id, values);
  ...
  return (
    <>
      <Breadcrumbs sx={{ mb: 4 }}>
        <Link href={`/admin/projects-programs/media/` + id}>
...
export default AddProgramMediaPage;
