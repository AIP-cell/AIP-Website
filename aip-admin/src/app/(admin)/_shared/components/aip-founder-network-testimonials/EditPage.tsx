...
export const EditAipFounderNetworkTestimonialPage = (values: any) => {
  const {
    register,
    reset,
    control,
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await FoundersNetworkTestimonialsApi.update(
        id,
        values,
        reqAuthHeader()
      );
...
export default EditAipFounderNetworkTestimonialPage;
