...
export const EditAipImpactTestimonialPage = (values: any) => {
  const {
    register,
    reset,
    control,
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await AipImpactTestimonialsApi.update(
        id,
        values,
        reqAuthHeader()
      );
  ...
  return (
    <>
      ...
      {/* Form Card */}
      <Card>
        <CardHeader title="Philanthropist speak" />
...
export default EditAipImpactTestimonialPage;
