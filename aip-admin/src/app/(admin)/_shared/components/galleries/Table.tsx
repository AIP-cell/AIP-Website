...
const GalleriesTable = () => {
      ...
      try {
        await dataTableApi
          ...
          .then((res) => {
            ...
            setRows(res.data.data?.galleries);
          });
      } catch (err) {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await GalleryApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await BlogsApi.updateStatus(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
      ...
      renderCell: ({ row }: { row: any }) => (
        ...
        >
          ...
          <Tooltip title="Delete">
            <IconButton onClick={() => deleteBlogs(row._id)} size="small">
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/galleries/edit/` + row._id}
            >
