...
export const AddProgramsGalleryPage = (
  { values }: any,
  {
    popperPlacement,
  }: {
  ...
  const {
    ...
    defaultValues: {
      galleryImage: values.galleryImages?.map((image: any) => ({
        date: dayjs(image.date).toDate(),
        title: image.title,
        image: image.image,
      })),
      galleryVideo: values.galleryVideos?.map((video: any) => ({
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await ProjectAndProgramsGalleryApi.add(values, id);
  ...
  return (
    <>
      ...
      {/* Form Card */}
      <Card>
        <CardHeader title="Programs Gallery Informations" />
...
export default AddProgramsGalleryPage;
