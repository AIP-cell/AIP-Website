...
const CuratedResourcesTable = () => {
  ...
  const fetchTableData = useCallback(
    async (sort: SortType, q: string) => {
      try {
        await dataTableApi
          .getCuratedResourcesTable({
            query: { sort, q, page: paginationModel.page + 1 },
          })
          .then((res) => {
            setTotal(res.data.data?.totalCount);
            setRows(res.data.data?.resources);
          });
      } catch (err) {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await AipCuratedResourceApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await AipCuratedResourceApi.updateStatus(
        id,
        reqAuthHeader()
      );
      toast.success(response?.data?.message);
  ...
  const handleFeaturedChange = async (id: any) => {
    try {
      const response = await AipCuratedResourceApi.updateFeaturedStatus(
        id,
        reqAuthHeader()
      );
      toast.success(response?.data?.message);
        ...
        return (
          ...
          >
            <Switch
              ...
              onChange={() => handleFeaturedChange(row._id)}
              inputProps={{ "aria-label": "controlled" }}
            />
          </div>
        );
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/curated-resources/edit/` + row._id}
            >
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/curated-resources/add`}
              >
                Add Resources
              </Button>
              &nbsp;
...
export default CuratedResourcesTable;
