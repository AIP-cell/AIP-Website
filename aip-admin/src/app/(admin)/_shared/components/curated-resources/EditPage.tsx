...
export const EditCuratedResourcesPage = (
  { values, data }: any,
  {
    popperPlacement,
  }: {
  ...
  const {
    ...
    defaultValues: {
      ...
      image: values.image,
      video: values?.video,
      galleryImage: values.galleryImages?.map((imageItem: any) => ({
        title: imageItem.title,
        date: dayjs(imageItem.date).toDate(),
        image: imageItem.image,
      })),
      galleryVideo: values.galleryVideos?.map((imageItem: any) => ({
        title: imageItem.title,
        date: dayjs(imageItem.date).toDate(),
  ...
  const defaultVideo = values?.video;
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      ...
      if (values.linkOrVideo === "video") {
        values.video = defaultVideo;
      }
      const response = await AipCuratedResourceApi.update(
        id,
        values,
        reqAuthHeader()
      );
...
export default EditCuratedResourcesPage;
