...
const BookTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await BookApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await BookApi.updateStatus(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
