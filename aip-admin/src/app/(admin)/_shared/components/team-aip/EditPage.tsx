...
export const EditTeamAipPage = (values: any) => {
  const router = useRouter();
  const data = values.values;

  const description = convertFromHTML(data.description || "");
  const {
    handleSubmit,
    register,
    control,
    ...
    defaultValues: {
      ...
      founders: values.values.founders,
      teamAIP: values.values.teamAIP,
      experts: values.values.experts,
      advisoryBoard: values.values.advisoryBoard,
      quote: values.values.quote,
      twitter: values.values.twitter,
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValue(name as keyof TyearSchema, checked, { shouldValidate: true });
  };
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      console.log(values);
      const response = await AipTeamApi.update(
        id,
        {
          ...values,
...
export default EditTeamAipPage;
