...
const AipTeamTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await AipTeamApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await AipTeamApi.updateStatus(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
  ...
  const columns: GridColDef<any>[] = [
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        ...
        if (row.coreFounder) categoriesToShow.push("Core Founder");
        if (row.founders) categoriesToShow.push("Founders");
        if (row.advisoryBoard) categoriesToShow.push("Advisory Board");
        if (row.teamAIP) categoriesToShow.push("Team AIP");
        if (row.experts) categoriesToShow.push("Experts");
        if (row.boardOfDirectors) categoriesToShow.push("Board Of Directors");

        const displayCategories = categoriesToShow.join(",  ");
        ...
        return (
          <div className="mui-table-row-cell">
            <Typography
              ...
              sx={{ color: "text.primary", fontWeight: 600 }}
            >
              {displayCategories || "No Categories"}
            </Typography>
          </div>
        );
      },
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/team-aip/people/edit/` + row._id}
            >
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/team-aip/people/add`}
              >
                Add Team
              </Button>
              &nbsp;
