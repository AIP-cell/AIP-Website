...
const ProgramTestimonialTable = () => {
  ...
  const fetchTableData = useCallback(
    async (sort: SortType, q: string) => {
      try {
        await dataTableApi
          .getProjectAndProgramTestimonialTable({
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await ProjectAndProgramsTestimonialApi.delete(
        id,
        reqAuthHeader()
      );
      ...
      if (response.data.success) {
        ...
        router.push(`/admin/projects-programs/testimonials/` + id);
        fetchTableData("asc", query);
      } else {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/projects-programs/testimonial/edit/` + row._id}
            >
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader
          title="Projects & Programs Gallery Of Voices"
          action={
            <div>
              <Button
                size="medium"
                variant="contained"
                component={Link}
                href={`/admin/projects-programs/testimonial/add/` + id}
              >
                Add Gallery Of Voices
              </Button>
              &nbsp;
            </div>
          }
...
export default ProgramTestimonialTable;
