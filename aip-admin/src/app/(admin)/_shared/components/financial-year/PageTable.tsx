...
const AipFinancialYearTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await YearApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await YearApi.status(id, reqAuthHeader());
      toast.success(response?.data?.message);
      fetchTableData("asc", query);
    } catch (err) {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/financial-report/financial-year/edit/` + row._id}
            >
  ...
  return (
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 4 }}>
        <Link href={`/admin/financial-year`}>
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/financial-report/financial-year/add`}
              >
                Add Year
              </Button>
              &nbsp;
...
export default AipFinancialYearTable;
