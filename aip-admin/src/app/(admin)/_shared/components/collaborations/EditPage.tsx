...
export const EditCollaborationPage = (
  { values }: any,
  {
    popperPlacement,
  }: {
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await CollaborationApi.updateCasestudies(id, values);
  ...
  const categoryOptions = Object.entries(categoryCollaboration).map(
    ([key, value]) => ({
      label: value,
      value: key,
    })
  );
  const TypeOfContentOptions = Object.entries(TypeOfContentCollaboration).map(
    ([key, value]) => ({
      label: value,
      value: key,
    })
