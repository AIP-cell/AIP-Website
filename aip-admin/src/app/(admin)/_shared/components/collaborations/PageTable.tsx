...
const CollaborationTable = () => {
      ...
      try {
        await dataTableApi
          ...
          .then((res) => {
            ...
            setRows(res.data.data?.collaborations);
          });
      } catch (err) {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await CollaborationApi.delete(id, reqAuthHeader());
  ...
  const handleStatusChange = async (id: any) => {
    try {
      const response = await CollaborationApi.updateStatusCasestudies(
        id,
        reqAuthHeader()
      );
      toast.success(response?.data?.message);
          ...
          <Box sx={{ display: "flex", alignItems: "center", marginTop: "7px" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/collaborations/city/` + row._id}
              >
                Manage
              </Button>
            </Box>
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/collaborations/edit/` + row._id}
            >
