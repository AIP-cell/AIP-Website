...
import NpoPeopleApi from "@/axios/NpoPeopleApi";
...
const NpoPeopleTable = () => {
  ...
  const deleteCategoryData = async (id: any) => {
    ...
    try {
      const response = await NpoPeopleApi.delete(id, reqAuthHeader());

      if (response.data.success) {
        toast.success(response.data.message);
        router.push("/admin/npo/people-we-partner-with");
        fetchTableData("asc", query);
      } else {
        ...
        >
          ...
          <Tooltip title="Edit">
            <IconButton
              size="small"
              component={Link}
              href={`/admin/npo/people-we-partner-with/edit/` + row._id}
            >
  ...
  return (
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 4 }}>
        <Link href={`/admin/npo/people-we-partner-with`}>
          <Typography>NPO</Typography>
        </Link>
          ...
          action={
            <div>
              <Button
                ...
                variant="contained"
                component={Link}
                href={`/admin/npo/people-we-partner-with/add`}
              >
                Add People
              </Button>
              &nbsp;
