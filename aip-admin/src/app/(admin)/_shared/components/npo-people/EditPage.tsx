...
export const EditNpoPeoplePage = (values: any) => {
  const {
    register,
    control,
    reset,
  ...
  const onSubmit = async (values: TyearSchema) => {
    try {
      const response = await NpoPeopleApi.update(id, values, reqAuthHeader());
  ...
  return (
    <>
      <Breadcrumbs sx={{ mb: 4 }}>
        ...
        <Typography>People we partner with</Typography>
      </Breadcrumbs>
...
export default EditNpoPeoplePage;
